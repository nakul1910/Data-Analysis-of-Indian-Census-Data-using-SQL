...
select a.district,a.state,a.literacy from(
select district, state, literacy, dense_rank() over(partition by state order by literacy desc) as rnk from  project..['Table 0$'] as p) as a
where a.rnk<=3;